...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.empty(self.hidden_size, self.embed_size * self.n_features))
        self.embed_to_hidden_bias = nn.Parameter(torch.empty(self.hidden_size, 1))
        self.dropout = nn.Dropout(p=self.dropout_prob)
        self.hidden_to_logits_weight = nn.Parameter(torch.empty(self.n_classes, self.hidden_size))
        self.hidden_to_logits_bias = nn.Parameter(torch.empty(self.n_classes, 1))
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.cat([torch.index_select(self.embeddings, 0, w[ind]).view(1, -1) for ind in range(w.shape[0])], dim=0)
    ...
    def forward(self, w):
        ...
        w_embeddings = self.embedding_lookup(w)
        ## Hidden layer 1 compute
        hidden = self.dropout(nn.ReLU()(self.embed_to_hidden_weight @ w_embeddings.T + self.embed_to_hidden_bias))
        logits = self.hidden_to_logits_weight @ hidden + self.hidden_to_logits_bias
        ### END YOUR CODE
        return logits.T
