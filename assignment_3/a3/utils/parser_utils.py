...
class Parser(object):
    ...
    def legal_labels(self, stack, buf):
        labels = ([1] if len(stack) > 2 else [0]) * self.n_deprel # Feasibility of making a left arc
        labels += ([1] if len(stack) >= 2 else [0]) * self.n_deprel # Feasibility of making a right arc
        labels += [1] if len(buf) > 0 else [0] # Feasibility of moving to stack
        return labels
