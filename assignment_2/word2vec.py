...
def naiveSoftmaxLossAndGradient(
    ...
    softmax_probability = softmax(np.matmul(outsideVectors, centerWordVec))
    loss = - np.log(softmax_probability[outsideWordIdx])
    gradCenterVec = - outsideVectors[outsideWordIdx,:] + np.sum(outsideVectors * softmax_probability.reshape(-1, 1), axis=0)
    gradOutsideVecs = centerWordVec * softmax_probability.reshape(-1, 1)
...
def negSamplingLossAndGradient(
    ...
    sigmoid_outside = sigmoid(np.dot(centerWordVec, outsideVectors[outsideWordIdx, :]))
    sigmmoid_negative_samples = sigmoid(-np.matmul(outsideVectors[negSampleWordIndices, :], centerWordVec))
    
    loss = - np.log(sigmoid_outside) - np.sum(np.log(sigmmoid_negative_samples))
    gradCenterVec = (
        (outsideVectors[outsideWordIdx, :] * (sigmoid_outside - 1)) + 
        np.sum(outsideVectors[negSampleWordIndices, :] * (1 - sigmmoid_negative_samples.reshape(-1, 1)) , axis=0)
    )
    
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx, :] = (sigmoid_outside - 1) * centerWordVec
    
    for idx, negSampleWordIndex in enumerate(negSampleWordIndices):
        gradOutsideVecs[negSampleWordIndex, :] += (1-sigmmoid_negative_samples[idx]) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWordIdx in outsideWordIndices:
        inc_loss, inc_gradCenterVec, inc_gradOutsideVecs = word2vecLossAndGradient(
        ...
        loss += inc_loss
        gradCenterVecs[centerWordIdx, :] += inc_gradCenterVec
        gradOutsideVectors += inc_gradOutsideVecs
