...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_length = random.randint(0, round(truncated_length/2))
        mask_start_idx = random.randint(0, truncated_length-mask_length)

        prefix = document[:mask_start_idx]
        masked_content = document[mask_start_idx:mask_length+mask_start_idx]
        suffix = document[mask_start_idx+mask_length:]
        """
        3. Rearrange these substrings into the following form:
        [prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] [pads]
        ...
        and output strings as Long tensors and return the resulting data point.
        """
        input = torch.tensor([self.stoi[c] for c in input], dtype=torch.long)
        output = torch.tensor([self.stoi[c] for c in output], dtype=torch.long)
